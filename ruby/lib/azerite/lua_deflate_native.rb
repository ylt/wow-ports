...
class LuaDeflateNative
  ...
  B64_ALPHABET = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'.freeze
  ...
  class << self
    def encode_for_print(str)
      ...
      padded = remainder == 0 ? str : str + "\0" * (3 - remainder)

      # Step 2: Reverse each 3-byte group
      reversed_input = padded.gsub(/.{3}/m) { |chunk| chunk.reverse }
      ...
      reversed_b64 = b64.gsub(/.{4}/) { |chunk| chunk.reverse }
    ...
    def decode_for_print(encoded_str)
      ...
      padded_b64 = remainder == 0 ? b64 : b64 + 'A' * (4 - remainder)

      reversed_b64 = padded_b64.gsub(/.{4}/) { |chunk| chunk.reverse }

      # Step 4: Pad to multiple of 4 with '=' for strict_decode64
      eq_remainder = reversed_b64.length % 4
      padded_for_decode = eq_remainder == 0 ? reversed_b64 : reversed_b64 + '=' * (4 - eq_remainder)
      ...
      unreversed = decoded.gsub(/.{3}/m) { |chunk| chunk.reverse }
      ...
      byte_count = full_groups * 3 + (leftover == 0 ? 0 : leftover - 1)

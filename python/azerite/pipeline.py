...
from .vuhdo_serializer import (
    deserialize as vuhdo_deserialize,
    serialize as vuhdo_serialize,
)
...
def find_format(addon: str, version: int) -> FormatConfig | None:
    return next(
        (f for f in AUTO_FORMATS if f["addon"] == addon and f["version"] == version),
        None,
    )
...
class Pipeline:
    ...
    @classmethod
    def decode(
        cls, export_str: str, addon: str | None = None, steps: list[Step] | None = None
    ) -> ExportResult:
        p = cls()
        ...
        if steps is None:
            if addon:
                ...
                pfx_step = next(
                    (s for s in steps if isinstance(s, dict) and "prefix" in s), None
                )
                if pfx_step:
                    match = next(
                        (f for f in AUTO_FORMATS if f["prefix"] == pfx_step["prefix"]),
                        None,
                    )
                    if match:
                        p.addon = match["addon"]
    ...
    def encode(
        ...
        if steps is None:
            if addon:
                ...
                if not fmt:
                    raise ValueError(
                        f"Unknown format: {export_result.addon} v{export_result.version}"
                    )
                steps = fmt["steps"]
    ...
    def detect_format(self) -> "Pipeline":
        fmt = next(
            (
                ...
                if f["prefix"]
                and isinstance(f["prefix"], str)
                and self.raw.startswith(f["prefix"])
            ),
            AUTO_FORMATS[-1],
        )
        self.format = fmt
    ...
    def extract_metadata(self) -> "Pipeline":
        text = (
            self.serialized.decode("latin-1")
            if isinstance(self.serialized, bytes)
            else self.serialized
        )
        meta_idx = text.find("^^::")
        if meta_idx != -1:

...
type HuffmanMap = Record<number, Record<number, string>>;

class LibCompress {
  static decompress(data: Buffer | Uint8Array): Buffer {
    ...
    switch (method) {
      case 1:
        return buf.slice(1);
      case 2:
        return LibCompress.decompressLZW(buf);
      case 3:
        return LibCompress.decompressHuffman(buf);
      default:
  ...
  static decompressLZW(buf: Buffer): Buffer {
  ...
  static decompressHuffman(buf: Buffer): Buffer {
    ...
    const map: HuffmanMap = {};
    let minCodeLen = Infinity;
    let maxCodeLen = 0;
    let symbolsRead = 0;
    let state: "symbol" | "code" = "symbol";
    let symbol = 0;
    ...
    while (true) {
      if (testLen <= bitfieldLen) {
        ...
        if (sym !== undefined) {
          ...
          testLen++;
          if (testLen > maxCodeLen)
            throw new Error("Huffman decode error: code too long");
        }
      } else {
...
function readCode(buf: Buffer, pos: number): [number, number] {
  const a = buf[pos]!;
  if (a < 250) return [a, 1];
  const count = 256 - a;
  let r = 0;
...
function unescape(code: number, codeLen: number): [number, number] {
  let unescaped = 0;
  let outPos = 0;
  let i = 0;
  while (i < codeLen) {

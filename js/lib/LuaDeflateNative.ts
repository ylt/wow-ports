...
class LuaDeflateNative {
  encodeForPrint(str: string): string {
    ...
    const padded =
      originalLength % 3 === 0
        ? originalLength
        : originalLength + (3 - (originalLength % 3));
  ...
  _decode(encodedStr: string): Buffer | null {
    ...
    const b64str =
      b64Chars.toString("latin1") + (r === 0 ? "" : r === 2 ? "==" : "=");
  ...
  decodeForPrint2(encodedStr: string): Uint8Array | null {
    if (typeof encodedStr !== "string") return null;
    const trimmed = encodedStr.trim();
...
export default LuaDeflateNative;

...
const KEY_TO_ABBREV: Record<string, string> = {};
for (const [k, v] of Object.entries(ABBREV_TO_KEY)) KEY_TO_ABBREV[v] = k;

function readLengthValue(str: string, pos: number): [number, string | null] {
...
class VuhDoSerializer {
  static deserialize(str: string): Record<string | number, unknown> {
  ...
  static serialize(table: Record<string | number, unknown>): string {
    let result = "";
    ...
    for (const [key, value] of Object.entries(table)) {
      ...
      if (typeof value === "string") {
        ...
        const nested = VuhDoSerializer.serialize(
...
export default VuhDoSerializer;

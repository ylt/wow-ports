...
describe('A. String Escaping (Serialize)', () => {
    ...
    test('A02: control chars 0x01–0x1D → chr(byte+64)', () => {
    ...
    test('A03: byte 30 (0x1E) → ~z (special case — 30+64=94=^ would corrupt parser)', () => {
        assert.strictEqual(WowAceSerializer.serialize('\x1E'), '^1^S~z^^');
    });

    test('A04: byte 31 (0x1F) → ~_', () => {
    ...
    test('A09: single-pass — multiple special chars, no double-escaping', () => {
        assert.strictEqual(WowAceSerializer.serialize('^~\x7F'), '^1^S~}~|~{^^');
    });
...
describe('C. Number Serialization', () => {
    ...
    test('C24: float wire format variants (0.1, -99.99, 1e-10)', () => {
...
describe('E. Float frexp Round-trips', () => {
    ...
    test('E40: round-trip very small float', () => {
        // JS cannot round-trip true subnormals (< 2^-1022): mantissa * 2^(e-53)
        // underflows to 0 for e < -1021. Minimum normal 2^-1022 round-trips correctly.
        assert.strictEqual(rt(2.2250738585072014e-308), 2.2250738585072014e-308);
    });
...
describe('L. Round-trips', () => {
    ...
    test('L70: boolean', () => {
        ...
        assert.strictEqual(rt(false), false);
    });

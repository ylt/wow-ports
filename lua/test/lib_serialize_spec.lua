...
------------------------------------------------------------------------
  ...
  describe("A. Nil", function()
    it("1. Serialize nil produces non-empty string", function()
      local s = ser(nil)
      assert.are.equal(type(s), "string")
      assert.truthy(#s > 0)
    end)

    it("2. Deserialize nil", function()
      assert.is_nil(roundtrip(nil))
    end)

    it("3. Nil round-trip via Deserialize API", function()
      local s = ser(nil)
      local ok, val = LibSerialize:Deserialize(s)
  ...
  describe("B. Integer encoding (small embedded)", function()
    it("4. Zero (embedded 1-byte)", function()
      assert.are.equal(roundtrip(0), 0)
    end)

    it("5. Positive small int 1", function()
      assert.are.equal(roundtrip(1), 1)
    end)

    it("6. Small int 127 (embedded 1-byte, max)", function()
      assert.are.equal(roundtrip(127), 127)
    end)

    it("7. Small negative -1 (embedded 2-byte)", function()
      assert.are.equal(roundtrip(-1), -1)
    end)

    it("8. Small negative -4095 (embedded 2-byte, near max)", function()
      assert.are.equal(roundtrip(-4095), -4095)
    end)

    it("9. Positive 128 (embedded 2-byte)", function()
      assert.are.equal(roundtrip(128), 128)
    end)

    it("10. Positive 4095 (embedded 2-byte, max)", function()
      assert.are.equal(roundtrip(4095), 4095)
    end)
  end)

  describe("C. Integer encoding (multi-byte)", function()
    it("11. 16-bit positive (4096)", function()
      assert.are.equal(roundtrip(4096), 4096)
    end)

    it("12. 16-bit positive max (65535)", function()
      assert.are.equal(roundtrip(65535), 65535)
    end)

    it("13. 16-bit negative (-4096)", function()
      assert.are.equal(roundtrip(-4096), -4096)
    end)

    it("14. 24-bit positive (65536)", function()
      assert.are.equal(roundtrip(65536), 65536)
    end)

    it("15. 24-bit positive max (16777215)", function()
      assert.are.equal(roundtrip(16777215), 16777215)
    end)

    it("16. 32-bit positive (16777216)", function()
      assert.are.equal(roundtrip(16777216), 16777216)
    end)

    it("17. 32-bit positive max (4294967295)", function()
      assert.are.equal(roundtrip(4294967295), 4294967295)
    end)

    it("18. 64-bit positive (4294967296)", function()
      assert.are.equal(roundtrip(4294967296), 4294967296)
    end)

    it("19. Large negative integer (-65536)", function()
      assert.are.equal(roundtrip(-65536), -65536)
    end)
  end)

  describe("D. Float encoding", function()
    it("20. Float 3.14 round-trip", function()
      assert.are.equal(roundtrip(3.14), 3.14)
    end)

    it("21. Float 0.1 round-trip", function()
      assert.are.equal(roundtrip(0.1), 0.1)
    end)

    it("22. Float -99.99 round-trip", function()
      assert.are.equal(roundtrip(-99.99), -99.99)
    end)

    it("23. Float 1e-10 round-trip", function()
      assert.are.equal(roundtrip(1e-10), 1e-10)
    end)

    it("24. Float 0.5 round-trip", function()
      assert.are.equal(roundtrip(0.5), 0.5)
    end)

    it("25. Positive infinity round-trip", function()
      assert.are.equal(roundtrip(math.huge), math.huge)
    end)

    it("26. Negative infinity round-trip", function()
      assert.are.equal(roundtrip(-math.huge), -math.huge)
    end)

    it("27. Max exact double integer (2^53) round-trip", function()
  ...
  describe("H. String refs (strings >2 bytes tracked on first use)", function()
    it("40. Repeated string uses ref — values preserved", function()
    ...
    it("41. String ref preserves value across 5 occurrences", function()
    ...
    it("42. Short strings (≤2 bytes) not tracked as ref — still round-trip", function()
  ...
  describe("I. Empty and small tables (embedded path)", function()
    it("43. Empty table round-trip", function()
      local result = roundtrip({})
      assert.are.equal(type(result), "table")
      assert.is_nil(next(result))
    end)

    it("44. Table with 1 key-value pair", function()
      assert.are.equal(roundtrip({a = 1}).a, 1)
    end)

    it("45. Table with 15 entries (embedded max)", function()
      local t = {}
  ...
  describe("J. Arrays (sequential 1-based integer keys)", function()
    it("46. Single-element array", function()
      assert.are.equal(roundtrip({"only"})[1], "only")
    end)

    it("47. Simple array [a, b, c]", function()
    ...
    it("49. Mixed-type array", function()
    ...
    it("50. Array with 16 elements (ARRAY_8 path)", function()
      local t = {}
  ...
  describe("K. Mixed tables (array + hash portions)", function()
    it("51. Mixed table: integer + string keys", function()
    ...
    it("52. Mixed table with various value types", function()
  ...
  describe("L. Table refs (repeated table references)", function()
    it("55. Shared sub-table round-trips correctly", function()
    ...
    it("56. Deserialized table refs point to the same table object", function()
      local inner = {value = 1}
      local result = roundtrip({a = inner, b = inner})
      result.a.value = 42
  ...
  describe("M. Variadic Serialize/Deserialize", function()
    it("57. Serialize multiple values", function()
    ...
    it("58. Serialize nil among values", function()
      local s = ser(1, nil, 3)
      local ok, a, b, c = LibSerialize:Deserialize(s)
      assert.are.equal(ok, true)
      assert.are.equal(a, 1)
  ...
  describe("N. Error handling", function()
    it("59. Deserialize corrupt data returns boolean result", function()
      local ok, _ = LibSerialize:Deserialize("not valid data \xff")
      assert.are.equal(type(ok), "boolean")
    end)

    it("60. IsSerializableType returns true for supported types", function()
    ...
    it("61. IsSerializableType returns false for functions", function()
      assert.falsy(LibSerialize:IsSerializableType(print))
    end)
  end)

  describe("O. Round-trips (comprehensive)", function()
    it("62. Complex nested structure", function()
    ...
    it("63. Table with boolean keys", function()
      local result = roundtrip({[true] = "yes", [false] = "no"})
      assert.are.equal(result[true], "yes")
      assert.are.equal(result[false], "no")
    end)

    it("64. Table with float key (non-integer → map portion)", function()
      local result = roundtrip({[1.5] = "fractional"})
      assert.are.equal(result[1.5], "fractional")
    end)

    it("65. String with null bytes round-trip", function()
      local s = "a\x00b\x00c"
      assert.are.equal(roundtrip(s), s)
    end)

    it("66. All 256 byte values in string", function()
      local s = ""
      for i = 0, 255 do s = s .. string.char(i) end
      assert.are.equal(roundtrip(s), s)
    end)
